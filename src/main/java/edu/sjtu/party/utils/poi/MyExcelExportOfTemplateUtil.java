 ...
 **/
public class MyExcelExportOfTemplateUtil extends BaseExportService {
    ...
    public Workbook createExcleByTemplate(TemplateExportParams params, Class<?> pojoClass, int sheetIndex,
        ...
        try {
            ...
            parseTemplate(wb.getSheetAt(sheetIndex), map, params.isColForEach());
            if (dataSet != null) {
                // step 4. 正常的数据填充
    ...
    private void hanlderSumCell(Sheet sheet) {
        for (TemplateSumHandler.TemplateSumEntity sumEntity : templateSumHandler.getDataList()) {
